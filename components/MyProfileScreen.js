...
import { View, Text, StyleSheet, Image, DrawerLayoutAndroid, ScrollView, ActivityIndicator, FlatList, TouchableOpacity, Button } from 'react-native';
...
const MyProfileScreen = ({ navigation }) => {
  ...
  const fetchFriends = async (userId) => {
    try {
      const response = await fetch(`https://amanda.capraworks.com/api/get_friends.php?user_id=${userId}`);
      const data = await response.json();
  ...
  >
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Image
        source={{ uri: `https://amanda.capraworks.com/api/${userDetails.cover_image}` }}
