...
import { View, Text, TextInput, Button, Image, TouchableOpacity, DrawerLayoutAndroid, StyleSheet, Alert, ScrollView } from 'react-native';
...
const ProfileScreen = ({navigation}) => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (user !== null) {
          ...
          setUserImage(parsedUser.user_image ? `https://amanda.capraworks.com/api/${parsedUser.user_image}` : null); // Full URL
          setCoverImage(parsedUser.cover_image ? `https://amanda.capraworks.com/api/${parsedUser.cover_image}` : null); // Full URL
        }
      } catch (e) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch('https://amanda.capraworks.com/api/user.php', {
        method: 'POST',
        body: formData,
      });
...
/>
    ...
    <Text onPress={() => handleImagePick('userImage')} style={styles.buttonPick}>
    ...
    <Text onPress={() => handleImagePick('coverImage')} style={styles.buttonPick}>
