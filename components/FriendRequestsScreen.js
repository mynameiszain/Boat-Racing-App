...
import { View, Text, FlatList, StyleSheet, Image, DrawerLayoutAndroid, ScrollView, Button, Alert, TouchableOpacity, ActivityIndicator } from 'react-native';
...
const FriendRequestScreen = ({navigation}) => {
  ...
  const fetchFriendRequests = async () => {
    try {
      if (receiverId) {
        const response = await fetch(`https://amanda.capraworks.com/api/accept_friend_request.php?receiver_id=${receiverId}`);
  ...
  const handleAction = async (requestId, action) => {
    try {
      ...
      const response = await fetch('https://amanda.capraworks.com/api/accept_friend_request.php', {
        method: 'POST',
        body: formData,
      });
...
/>
    ...
    <View style={{ flex: 1, padding: 16, marginTop: 100, }}>
      <Text style={{ fontSize: 24, marginBottom: 16,color:'#1D1852' }}>Friend Requests</Text>
      <FlatList
        data={friendRequests}
        keyExtractor={item => item.id.toString()}
        renderItem={({ item }) => (
          <View style={{ marginBottom: 16, padding: 16, backgroundColor: '#f9f9f9', borderRadius: 8 }}>
            <Text style={{ fontSize: 18, textTransform: 'capitalize', color: '#1D1852' }}>{item.sender_name}</Text>
            <View style={{ flexDirection: 'row', marginTop: 8 }} >
              <Button title="Accept" onPress={() => handleAction(item.id, 'accept')} color= '#1D1852' />
            <View style={{ width: 10,backgroundColor: 'white', }} />
              <Button title="Reject" onPress={() => handleAction(item.id, 'reject')} style={styles.reject} color="red" />
            </View>
          </View>
        )}
        ListEmptyComponent={<Text style={{color:'#1D1852',}}>No friend requests found.</Text>}
      />
    </View>
    </ScrollView>
    </DrawerLayoutAndroid>
